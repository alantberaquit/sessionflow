...
const formatRegistration = (registration) => ({
  ...
  breakoutSelections:
    registration.breakoutSelections.map((selection) => ({
      breakoutBlock: selection.breakoutBlock,
      breakoutSession: selection.breakoutSession,
    })),
...
const isParticipantProfileComplete = (profile) =>
  Boolean(
...
const validateSelectionShape = (selections) => {
  const errors = {};
  ...
  for (
    ...
    if (
      ...
      errors.breakoutSelections =
        `Selection ${index + 1} has an invalid breakout block.`;
    ...
    if (
      ...
      errors.breakoutSelections =
        `Selection ${index + 1} has an invalid breakout session.`;
...
const validateSelectionRelationships = async (
  ...
  const blockIds = selections.map(
    (selection) => selection.breakoutBlock,
  );

  const sessionIds = selections.map(
    (selection) => selection.breakoutSession,
  );
  ...
  for (const selection of selections) {
    ...
    if (
      session.breakoutBlock.toString() !== blockId
    ) {
...
export const saveRegistrationDraft = async (
  ...
  try {
    ...
    const selections =
      request.body.breakoutSelections ?? [];

    const validationErrors =
      validateSelectionShape(selections);
...
export const submitRegistration = async (
  ...
  try {
    ...
    const selectionCountsByBlock = new Map();
    ...
    selections.forEach((selection) => {
      ...
      selectionCountsByBlock.set(
        blockId,
        (selectionCountsByBlock.get(blockId) ||
          0) + 1,
      );
    });
    ...
    for (const block of breakoutBlocks) {
      ...
      const selectedCount =
        selectionCountsByBlock.get(blockId) || 0;

      if (
        selectedCount < block.minimumSelections
      ) {
        response.status(400).json({
          message:
            "Complete all required breakout selections before submitting.",
      ...
      if (
        selectedCount > block.maximumSelections
      ) {
        response.status(400).json({
          message:
